...
class PaymentController {
    ...
    async processPayment(req, res) {
        try {
            ...
            switch (paymentMethod) {
                case 'chapa':
                    paymentResult = await this.processChapa(order, paymentDetails, req.user);
                    break;
                case 'paypal':
                    paymentResult = await this.processPayPal(order, paymentDetails);
                    break;
                case 'telebirr':
                    paymentResult = await this.processTelebirr(order, paymentDetails, req.user);
                    break;
                case 'cbe':
                    paymentResult = await this.processCBE(order, paymentDetails, req.user);
                    break;
     ...
     */
    processChapa = async (order, paymentDetails, user) => {
        try {
     ...
     */
    processPayPal = async (order, paymentDetails) => {
        try {
     ...
     */
    processTelebirr = async (order, paymentDetails, user) => {
        try {
            const baseUrl = process.env.BASE_URL || 'http://localhost:5000';
     ...
     */
    processCBE = async (order, paymentDetails, user) => {
        try {
            const baseUrl = process.env.BASE_URL || 'http://localhost:5000';
     ...
     */
    processCashOnDelivery = async (order) => {
        return {
            success: true,
            status: 'completed',
     ...
     */
    processBankTransfer = async (order, paymentDetails) => {
        return {
            success: true,
            status: 'completed',
     ...
     */
    async verifyChapaPayment(req, res) {
        try {
