...
async function disableChapa() {
    try {
        // Connect to MongoDB
        await mongoose.connect(process.env.MONGODB_URI);
        console.log('✅ Connected to MongoDB');

        // Disable Chapa payment method
        ...
        if (result.modifiedCount > 0) {
            console.log('✅ Chapa payment method disabled successfully');
        } else {
            console.log('⚠️ Chapa payment method not found or already disabled');
        }
